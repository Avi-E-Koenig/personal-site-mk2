    ...
    <div className="section">
      <div className="container-content">
        <header className="mb-10">
          ...
          <h1 className="text-h1 md:text-h1-md text-text-primary mb-3">{t('title')}</h1>
          <p className="text-text-secondary max-w-2xl">{t('intro')}</p>
        </header>

        <div className="space-y-6">
          {caseStudies.map((cs) => (
            <article key={cs.slug} id={cs.slug} className="card p-6 sm:p-8 scroll-mt-24">
              ...
              <div className="space-y-5">
                <Field label={t('fields.situation')}>{cs.situation}</Field>
                <Field label={t('fields.approach')}>{cs.approach}</Field>
                <Field label={t('fields.outcome')}>{cs.outcome}</Field>
              </div>
